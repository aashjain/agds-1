...
export const backgroundFragmentShader = /* glsl */ `
    ...
    void main() {
        ...
        float t = uTime * 4.2 + uScroll * 115.0;
        ...
        float mask1 = pow(fbm(st1 + f1 * 2.8 - vec2(t * 0.2, 0.0)), 2.0) * 3.1;

        // Shape 2: Blazing Orange blurs (color1)
        float f2 = fbm(st2 - vec2(t * 0.1, t * 0.2));
        float mask2 = pow(fbm(st2 + f2 * 2.2 + vec2(0.0, t * 0.16)), 2.4) * 3.4;
        ...
        finalColor *= cornerFade * (0.58 + uScroll * 0.38);

        // Deep dark background base
        vec3 baseBg = vec3(0.008, 0.010, 0.028);
