...
export const particleVertexShader = /* glsl */ `
    ...
    void main() {
        ...
        float dnaRadius = 1.18;
        float strandThickness = 0.28;
        ...
        targetWavePos.x += sin(targetWavePos.z * 0.035 + uTime * 0.7) * 1.8;
        ...
        float galaxyR = pow(gRnd1, 2.15) * 56.0 + pow(gRnd2, 3.0) * 34.0;
        ...
        float theta = baseAngle + swirl + dTheta + (uTime * 0.28);
        ...
        opacityMultiplier = mix(opacityMultiplier, 0.32, galaxyPhase);
        ...
        vec3 cBottom = vec3(0.10, 0.78, 1.0);
        vec3 cTop = vec3(0.86, 0.64, 0.30);
        ...
        vec3 cNebula = vec3(0.66, 0.54, 1.0);
        vec3 galaxyColor = mix(mix(cBottom, cNebula, 0.45), cTop, clamp(orangeMix, 0.0, 1.0));
...
export const particleFragmentShader = /* glsl */ `
    ...
    void main() {
        ...
        float core = smoothstep(0.18, 0.0, ll);
        vec3 litColor = mix(vColor, vec3(1.0, 0.97, 0.86), core * 0.34);
        gl_FragColor = vec4(litColor, vEdgeFade * pointAlpha * 0.86);
    }
`;
